...
public class deepFake : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit, interactionDistance))
        {
            ...
            {
                tweetMaterials = Resources.LoadAll<Material>("ai");
        ...
        scoreText.text = "DF-Score: " + playerScore;
    }
    ...
    void CheckTweet(char playerPick)
    {
        if (!gameOver)
        {
            ...
            if (isReal && playerPick == 'H')
            {
                Debug.Log("Correct! The tweet is real.");
                playerScore += 1;
            }
            else if (!isReal && playerPick == 'G')
            {
